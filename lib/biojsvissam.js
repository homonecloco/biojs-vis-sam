...
var BAMViewer = function (options) {
    ...
    this.on('onCentralBaseChanged', function(pos){
 		console.log(pos); // will print "some data"
 		this.set_central_base(pos);
	});
...
BAMViewer.prototype.build_aln_div = function(aln){
	...
	new_div.id = this.opt.target + "_" + aln.full_id();
    ...
    BAMViewer.prototype._draw_seq = function(new_div, aln){
    	...
    	var seq_len = aln.parsed_seq.length;
...
var BAMRegionSelector = function (options){
	...
	target=this.opt.target;
	  ...
	  self.on('onCentralBaseChanged', function(data){
